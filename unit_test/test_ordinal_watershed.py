...
class WatershedSegmentationTestConfig(WatershedConfig):
    ...
    loss_function_energy = "tversky_g2"
    epochs: int = 2
...
def train_segmentation():
    ...
    trainer = OrdinalWatershedTrainer(config)
    trainer.train_net()
...
def predict_segmentation(config, run_dir, save_dir):
    ...
    predictor = OrdinalWatershedPredictor(config, OrdinalWatershedTrainer)
...
def test_ordinal_watershed_segmentation():
    config, run_dir = train_segmentation()

    save_dir = "unit_test/test_owsegmentation"
...
if __name__ == "__main__":
    test_ordinal_watershed_segmentation()
