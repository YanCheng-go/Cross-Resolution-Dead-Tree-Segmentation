...
class PretrainedUNet(nn.Module):
    ...
    def __init__(self,
        ...
        # self.in_normalizer = Normalize(mean=[0.485, 0.456, 0.406, 0.4], std=[0.229, 0.224, 0.225, 0.2])

        self.last_conv = nn.Conv2d(16, n_classes, 1).eval()
    ...
    def forward(self, img):
        x = img   # x = self.in_normalizer(img)
        deepfeats = self.encoder(x)
        out = self.decoder(*deepfeats)
    ...
    def get_save_dict(cls, model, optm=None):
        ...
        st_dict['model_type'] = "pretrained_unet"
