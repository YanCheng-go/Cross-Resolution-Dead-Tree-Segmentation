...
def binary_focal_loss_with_logits(
    ...
    Args:
        ...
        weight: weights for classes with shape :math:`(num\_of\_classes,)`.
        pixel_weight: weights for each pixel with shape :math:`(N, C, *)`.
    ...
    if pixel_weight is not None:
        KORNIA_CHECK_IS_TENSOR(pixel_weight, "pixel_weight must be Tensor or None.")
        KORNIA_CHECK(
            pixel_weight.device == pred.device,
            f"pixel_weight and pred must be in the same device. Got: {pixel_weight.device} and {pred.device}",
        )
        loss_tmp = pixel_weight * loss_tmp
...
class BinaryFocalLossWithLogits(nn.Module):
    ...
    def forward(self, pred: Tensor, target: Tensor, weights) -> Tensor:
        return binary_focal_loss_with_logits(
            pred, target, self.alpha, self.gamma, self.reduction, self.pos_weight, self.weight, weights
        )
